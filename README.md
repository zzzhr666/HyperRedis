...
当前项目重点是 `HyperRedisCore`：一个不依赖完整网络服务器的 Redis-like 静态核心库。它已经包含底层数据结构、Redis 对象模型、多 DB 存储管理、过期机制、RDB-like 快照持久化、AOF-like 追加日志与重放、AOF rewrite、RESP 请求解析与响应序列化、命令执行协调层、服务端视角的客户端会话状态、socket I/O 适配层，以及一个可测试的 `poll`-based 文件事件循环。完整 TCP 服务器、listen/accept 生命周期、后台持久化任务和性能基准仍在后续计划中。
...
HyperRedis/
├── include/hyper/
│   ├── time.hpp                    # 统一的过期时间、毫秒和 Unix milliseconds 类型别名
...
│   └── server/                     # 命令核心、客户端会话与事件基础设施
│       ├── resp_value.hpp          # RESP 响应值模型
│       ├── resp_codec.hpp          # RESP 命令解析与响应序列化
│       ├── client_context.hpp      # 客户端当前 DB 上下文
│       ├── client_session.hpp      # 服务端视角下的单客户端会话状态
│       ├── client_socket_io.hpp    # fd read/write 与 ClientSession 缓冲区适配
...
┌───────────────────────────────▼──────────────────────────────┐
│                    Client / I/O / DB Layer                   │
│  ClientSession          client_socket_io     RedisManager    │
│  - query buffer         - fd read/write      - 多 DB 生命周期管理 │
│  - reply buffer         - WouldBlock/Closed  - FLUSHDB/ALL   │
│  - RedisClientContext                                        │
└───────────────────────────────┬──────────────────────────────┘
                                │
┌───────────────────────────────▼──────────────────────────────┐
...
| **time.hpp**           | 统一 `ExpireClock`、`ExpireTimePoint`、`Milliseconds` 和 `UnixMilliseconds` |
| **RedisClientContext** | 保存客户端当前 DB 选择                                    |
| **ClientSession**      | 服务端视角的单客户端会话状态，维护 fd 标识、query buffer、reply buffer 和当前 DB 上下文 |
| **ClientSocketIo**     | 将 fd read/write 映射到 `ClientSession` 缓冲区，区分成功、阻塞、关闭和错误状态 |
...
| 命令响应写入客户端输出缓冲 | `RespValue` 建模响应，`ClientSession` 保存 reply buffer | 命令层可脱离网络层测试，也能接入 socket 写回 |
...
当前项目还没有完整 TCP server，因此命令执行器仍不直接拥有 socket。服务端视角的单连接状态已经由 `ClientSession` 承担：它维护 query buffer、reply buffer、当前 DB 上下文和 fd 标识，并把 RESP2 命令数组交给 `parseRespCommand` 解析成参数数组：
...
- `ClientSession` 可以独立测试 query/reply buffer 消费、二进制 bulk 响应和连续命令处理
- `client_socket_io` 可以独立测试 read/write、`WouldBlock`、peer close 和部分写入消费
...
事件循环目前是服务器层的基础设施。`EventLoop` 维护 fd 到 readable / writable callback 的注册表，每次 `runOnce` 都从注册表构造 `pollfd` 快照，通过 `poll()` 等待就绪事件，并在分发前重新检查 fd 当前是否仍然存在、是否仍关注该事件。这保证了 callback 内调用 `removeFileEvent` 是安全的。它不拥有 fd，也不负责 socket 创建、关闭或命令执行；后续最小 `RedisServer` 会持有 fd 到 `ClientSession` 的映射，并在 readable / writable callback 中调用 `client_socket_io`。
...
- [x] `time.hpp` 统一项目内过期时间、毫秒和 Unix milliseconds 类型别名
- [x] `ClientSession` 维护服务端视角的单客户端 fd、query buffer、reply buffer 和当前 DB 上下文
- [x] `ClientSession::processInput` 接入 RESP parser、`CommandProcessor` 和 RESP 响应序列化
- [x] 支持半包保留、连续命令消费、协议错误响应和二进制 bulk reply
- [x] `client_socket_io` 将 fd read/write 适配到 `ClientSession`，处理 `EINTR` retry、`WouldBlock`、peer close 和写缓冲消费
...
- 客户端会话与 socket I/O：query/reply buffer、半包、连续命令、二进制 bulk reply、部分写入消费、peer close 和非阻塞 `WouldBlock`
...
| 维度 | Redis 原版 | HyperRedis |
| --- | --- | --- |
| 网络与命令 | 命令直接作用于客户端连接和 DB | `ClientSession` 持有连接状态，`CommandProcessor` 协调执行与 AOF |
| 请求读取 | 从客户端 query buffer 解析 RESP | `ClientSession` 累积 query buffer，`parseRespCommand` 解析 array-of-bulk-strings |
| 回复写入 | 写入 client reply buffer | `RespValue` 由 `serializeRespValue` 序列化后进入 `ClientSession` reply buffer |
| 测试方式 | 集成式服务测试为主 | 命令执行、会话 buffer 和 socket I/O 可分层单测 |
...
后续任务按《Redis 设计与实现》的学习节奏，从客户端会话和文件事件循环继续过渡到服务器生命周期和时间事件。`BGSAVE` 与 `BGWRITEAOF` 暂不作为下一步直接实现目标，它们依赖后台任务状态、serverCron 检查、rewrite buffer 和原子替换流程，更适合在服务器骨架稳定后推进。

### 近期任务流

- [x] 统一时间类型到 `hyper/time.hpp`
- [x] 设计客户端输入/输出缓冲对象：query buffer、reply buffer、当前 DB、RESP command 解析结果
- [x] 接入 RESP codec 到客户端 query buffer，支持半包和连续命令
- [x] 增加客户端 socket I/O 适配层：read query、write reply、处理阻塞/关闭/错误状态
...
- [ ] 将 `EventLoop` 接入 `RedisServer` 的 fd -> `ClientSession` 映射
